...
static inline const char* allowed_env_vars[] = {
	"TERM=",
	nullptr
};

static inline bool allow_env_var(const char *var)
{
...
struct rv_emulator
{
	...
	*/
...
#ifdef RECOGNI
	/* Start the executable with the given priv processor template */
	template <typename P>
	void setup_priv(P& proc)
	{
		/* setup floating point exception mask */
		fenv_init();
	...
	/* Finish the executable with the given priv processor template */
	template <typename P>
	void fini_priv(P& proc)
	{
	}
